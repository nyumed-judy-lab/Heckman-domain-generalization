...
class ClassificationExample(BivariateExample):
    def __init__(self,
                 input_covariance_matrix: torch.FloatTensor,
                 outcome_intercept: torch.FloatTensor = torch.FloatTensor([1.0, ]),
                 outcome_coef: torch.FloatTensor = torch.FloatTensor([4.5, 3.0]),
                 num_train_domains: int = 2,
                 num_test_domains: int = 1,
                 selection_feature_index: typing.List[int] = [1, ],
                 alpha_prior_mean: typing.Union[float, typing.List[float]] = 5.0,
        ...
        super(ClassificationExample, self).__init__(
            ...
            sigma_s=1.0,  # fixed based on assumption
            sigma_y=1.0,  # fixed based on assumption
            rho=rho,
            population_size=population_size,
            sampling_rate_per_domain=sampling_rate_per_domain
        )
    
    def _generate_population(self) -> typing.Dict[str, torch.Tensor]:
        """Generate true population (classification)."""

        # 1. Generate input covariates; (N, P)
        x = self._input_distribution.sample((self.population_size, ))

        # 2. Generate correlated errors from a bivariate normal distribution; (N, 2)
        ...
        y = self.outcome_intercept + torch.matmul(x, self.outcome_coef) + errors_y
        y = y.gt(0).long()
    ...
    def error_covariance_matrix(self) -> torch.FloatTensor:
        return torch.tensor([[1, self.rho],
                             [self.rho, 1]])
