...
import argparse
from networks import SharedHeckmanNetwork
from models import HeckmanDGBinaryClassifier
...
train_dat, test_dat = train_test_split(dat, stratify=dat['SITE']+dat['CVD'].astype(str), test_size=0.2, random_state=args.seed)
...
covariate_list = train_dat.columns.drop(['SSID', 'SITE', 'CVD'])
...
for train_sites in combinations(sites, args.num_domains):
    ...
    train_dat, val_dat = train_test_split(train_dat, stratify=train_dat['SITE'] + train_dat['CVD'].astype(str), test_size=0.2, random_state=args.seed)
    ...
    max_epoch = 100
    network = SharedHeckmanNetwork([tr_x.shape[1], 128, 64, args.num_domains+1], dropout=0.25, batchnorm=True, activation='ReLU')
    optimizer = partial(torch.optim.SGD, lr=1e-2, weight_decay=1e-2, momentum=0.9)
    scheduler = partial(torch.optim.lr_scheduler.MultiStepLR, milestones=[25, 50, 75, 100], gamma=.2)
    ...
    plt.tight_layout()
    plt.savefig(f"dx_figs/heckmanDG_shared_{'+'.join(train_sites)}_{args.seed}.pdf")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"dx_figs/heckmanDG_shared_{'+'.join(train_sites)}_{args.seed}_probits.pdf")
    plt.close()
...
pd.DataFrame(simple_stack, index=train_domains, columns=sites + ['Internal', 'External', 'All']).to_csv(f'results/heckmanDG_shared_{args.num_domains}_{args.seed}.csv')
