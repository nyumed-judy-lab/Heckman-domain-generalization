    ...
    for epoch in epochs:
        for w in id_proportion:
            for selection in model_selection_list:
                args.seed = seed # args.seed = 0
                args.w = w # args.w = 0.0
                args.model_selection = selection # args.model_selection = 'metric'#, 'loss'
                args.epochs = epoch # args.epochs = 5 'loss'#, 'metric'
                result_name = f'{args.data}_{args.seed}_w_{args.w}_selection_{args.model_selection}'
                result_name = f'{args.data}_{args.seed}_epoch_{args.epochs}_w_{args.w}_selection_{args.model_selection}_augmentation_{args.augmentation}'
                fix_random_seed(args.seed)
                
                # args.seed = 0 # id_proportion
                ...
                print('id_valid_loader', len(id_valid_loader))
                print('ood_valid_loader', len(ood_valid_loader))
                ...
                if False:
                    ...
                    path = 'results/results-old-2/models/camelyon17_0_epoch_5_w_0.0_selection_metric_model.pth'
                    state_dict = torch.load(path)
                    # state_dict = torch.load('results/models/camelyon17_0_epoch_5_w_0.0_selection_loss_model.pth')
                    # Load the saved state dictionary into the model
                    model.network.load_state_dict(state_dict)
                    
                    
                    
                #%% 4. Results Analysis tmux attach -t 
